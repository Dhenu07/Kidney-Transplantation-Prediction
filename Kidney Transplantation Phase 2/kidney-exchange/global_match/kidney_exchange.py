...
def optimize_length(cycles, vertices, dirname, coef=[], ilp=True):
    """Optimize kidney exchange cycles based on length"""
    prob = LpProblem("KIDNEY_EXCHANGE", LpMaximize)

    # Define decision variables
    vars = {i: LpVariable(f"c_{i}", 0, 1, LpBinary if ilp else LpContinuous) for i in range(len(cycles))}

    # Objective function: maximize sum of coefficients (or cycle count if coef is empty)
    prob += lpSum(vars[i] * (coef[i] if coef else 1) for i in range(len(cycles)))

    # Constraints: Ensure each vertex appears in at most one cycle
    for v in vertices:
        prob += lpSum(vars[i] for i, cycle in enumerate(cycles) if v in cycle) <= 1, f"v_{v}"

    # Solve the problem
    prob.solve()

    print("SUM IS", sum(var.varValue for var in vars.values()))
    return [var.varValue for var in vars.values()]


def optimize_weight(cycles, vertices, weight, dirname, ilp):
    """Optimize kidney exchange cycles based on weight"""
    prob = LpProblem("KIDNEY_EXCHANGE", LpMaximize)

    # Define decision variables
    vars = {i: LpVariable(f"c_{i}", 0, 1, LpBinary if ilp else LpContinuous) for i in range(len(cycles))}

    # Objective function: maximize cycle weights
    prob += lpSum(vars[i] * weight[tuple(cycles[i])] for i in range(len(cycles)))
...
def removechains(cycles):
    """Remove non-cyclic chains from the exchange cycle"""
    return [cycle for cycle in cycles if cycle[0] == cycle[-1]]


def maximize_pairwise_exchange(cycles, vertices, dirname, edges, ilp):
    """Maximize pairwise kidney exchanges"""
    coef = []
    precomputation = CyclePrecomputation()
...
def maximize_total_weight(cycles, vertices, cycle_wt, dirname, ilp):
    """Maximize the total weight of the exchange"""
    solution_values = optimize_weight(cycles, vertices, cycle_wt, dirname, ilp)
    print(solution_values)
    return solution_values
